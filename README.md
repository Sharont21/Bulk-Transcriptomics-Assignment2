...
Despite these advances, comprehensive stage-specific transcriptomic characterization of flor yeast during distinct phases of velum formation remains limited, and the coordinated regulatory programs underlying biofilm maturation are not fully understood [1]. The objective of this study was to characterize gene expression changes across three stages of velum development to identify molecular pathways associated with biofilm maturation. The stages include early biofilm formation (Stage 1), thin biofilm (Stage 2), and mature velum (Stage 3).
...
Raw RNA-sequencing data were obtained from the NCBI Sequence Read Archive (SRA) under BioProject accession PRJNA592304. Accession numbers were retrieved and downloaded using SRA Toolkit v3.0.9 [10]. The `prefetch` utility was used to download SRA files, and `fasterq-dump` was used to convert SRA files to FASTQ format. Sequencing read quality was assessed using FastQC (v0.12.1) [11]. FastQC reports were generated for each sample to evaluate per-base sequence quality, GC content, sequence duplication levels, and adapter contamination. FastQC reports were visually inspected to confirm that read quality was sufficient for downstream pseudoalignment-based quantification.

## Transcript Quantification
Transcript-level quantification was performed using Salmon (v1.10.2) in selective-alignment mode [12]. A reference transcriptome for _Saccharomyces cerevisiae_ (R64-1-1) was obtained from Ensembl and corresponds to the extensively curated S288C reference genome. This reference was selected because it provides standardized gene identifiers and functional annotations compatible with downstream Gene Ontology (GO) and KEGG enrichment analyses. The reference genome was used to construct the Salmon index using `salmon index`. Quantification was performed using `salmon quant` with the `--validateMappings` option enabled to improve mapping accuracy via selective alignment. Single-end reads were supplied using the `-r` argument, and library type was explicitly specified as unstranded `-l U`. Four CPU threads were allocated per sample (`-p 4`). Output for each sample was written to independent directories containing transcript-level abundance estimates in `quant.sf` format. 
...
Across all comparisons, significant genes were distributed across a broad range of effect sizes, with both moderate and large magnitude log2 fold changes observed. The volcano plots illustrate the magnitude and statistical significance of gene expression changes throughout stage-dependent biofilm development.
...
This transcriptomic analysis reveals extensive and coordinated gene expression remodeling across stages of flor yeast velum development. Principal component analysis (Figure 1) demonstrated clear separation of samples by developmental stage, with PC1 explaining the majority of variance. The strong clustering of biological replicates within each stage indicates high reproducibility and confirms that transcriptional identity shifts progressively from Stage1 to Stage3. The separation along PC1 suggests that biofilm maturation is the dominant driver of transcriptional variation in this dataset.

Differential expression analysis further supported this global remodeling. Volcano plots (Figure 2) revealed thousands of significantly regulated genes across pairwise comparisons, with substantial numbers of both upregulated and downregulated transcripts. The largest transcriptional shift was observed between Stage3 and Stage1, consistent with the PCA results and reflecting the progressive transition from early biofilm formation to mature velum. The balanced distribution of up- and downregulated genes suggests that maturation involves coordinated activation and repression of distinct biological programs rather than unidirectional expression changes. Such large-scale bidirectional regulation is consistent with the idea that biofilm development represents a developmental reprogramming event involving redistribution of cellular resources between growth-associated and adaptive processes [26]. 

Among genes associated with biofilm formation, the adhesin-encoding gene FLO11 is of particular importance. FLO11 encodes a glycosylphosphatidylinositol (GPI)-anchored cell surface glycoprotein that mediates cell–cell adhesion, surface attachment, and flocculation in _Saccharomyces cerevisiae_ [27]. Previous studies have shown that increased FLO11 expression enhances cell surface hydrophobicity and promotes aggregation, facilitating stable biofilm formation at air–liquid interfaces [27]. These properties are especially relevant in flor yeast, where flotation and velum formation depend on coordinated adhesion and multicellular organization [2]. Although FLO11 alone does not account for the full structural complexity of mature biofilms, its regulation likely represents a key initiating step in the transition from planktonic growth to structured velum development, functioning within the broader transcriptional remodeling observed across developmental stages [1,27].

Importantly, FLO11 expression is subject to glucose repression and becomes activated following sugar depletion at the end of alcoholic fermentation [28]. Experimental studies have demonstrated that overexpression of FLO11 is sufficient to induce flor formation in otherwise non-flor laboratory strains, indicating that derepression of this adhesin represents a critical regulatory switch [28]. In the context of biological wine aging, where fermentable sugars are exhausted and ethanol becomes the primary carbon source, relief of glucose repression likely promotes FLO11-mediated adhesion and flotation. Unlike many microbial biofilms that are embedded in a polysaccharide-rich extracellular matrix, the flor consists primarily of buoyant, highly hydrophobic cells at the air–liquid interface, further emphasizing the central role of FLO11-driven cell surface properties in velum development [28].

Heatmap visualization of the top differentially expressed genes (Figure 3) further illustrates these coordinated expression patterns. Genes selected from the Stage3 vs Stage1 comparison show clear segregation of samples by developmental stage, with Stage2 often exhibiting intermediate expression levels. This intermediate pattern supports a progressive developmental trajectory rather than abrupt stage transitions. The consistent clustering of samples by stage reinforces the robustness of stage-specific transcriptional programs identified through differential expression analysis.

Pathway-level analysis provides mechanistic insight into these transcriptional changes. Likelihood ratio test (LRT) clustering identified 3,900 genes exhibiting significant stage-dependent regulation, which grouped into four distinct expression clusters (Figure 4). Functional enrichment analysis of these clusters revealed biologically coherent programs, including translation- and ribosome-associated processes, lipid metabolic pathways, mitochondrial functions, and RNA regulatory processes. The enrichment of translation-related genes within early-stage clusters and metabolic pathways within later-stage clusters suggests a coordinated shift in cellular priorities during biofilm maturation. This transition from biosynthetic and growth-associated processes toward metabolic and stress-adaptive pathways indicates that velum development involves dynamic remodeling of central metabolism rather than isolated transcriptional changes [26].

Gene Set Enrichment Analysis (GSEA) further clarified pathway directionality. The Ribosome pathway was strongly negatively enriched in the Stage3 vs Stage1 comparison (Figure 6), indicating coordinated downregulation of translational machinery in the mature velum. The enrichment curve demonstrates that ribosomal genes cluster toward the Stage1-enriched end of the ranked list, supporting reduced protein synthesis in Stage3. In contrast, metabolic pathways such as fatty acid metabolism and glycolysis/gluconeogenesis were positively enriched in Stage3 (Figure 5), indicating activation of metabolic remodeling during maturation. Together, these findings suggest a transition from a growth-oriented transcriptional program in early biofilm formation to a metabolically adapted, stress-resilient state in the mature velum.

This shift from translational activity toward metabolic specialization is consistent with the physiological conditions of biological wine aging [3]. Flor yeast in mature biofilms experience nutrient limitation, oxidative stress, and high ethanol concentrations [1]. Suppression of ribosomal genes may reflect reduced proliferative demand under these conditions, while upregulation of lipid and oxidative metabolic pathways likely supports membrane stability, respiration, and long-term survival. Since oxygen and nutrients are unevenly distributed within biofilms, some of the metabolic shifts observed in this study may represent adaptations of cells located in different regions of the velum [26]. These transcriptional changes align with previous genomic and proteomic evidence indicating that flor yeast strains possess specialized adaptations that promote biofilm formation and oxidative metabolism [3]. 

Several limitations should be considered. This study used bulk RNA-seq data, which measures the average gene expression across all cells in the biofilm and does not capture differences between individual cells. As a result, potential variation between subpopulations within the velum may be overlooked. In addition, although pathway enrichment analyses identify coordinated changes in gene expression, transcript levels do not necessarily reflect actual enzyme activity or metabolic rates. Future studies combining transcriptomic data with proteomic or metabolomic analyses would provide a more complete understanding of how these gene expression changes affect cellular function during biofilm maturation.

Overall, the integration of global expression analysis (Figure 1), differential gene regulation (Figure 2), expression pattern visualization (Figure 3), clustering-based functional analysis (Figure 4), and pathway-level enrichment (Figures 5 and 6) demonstrates that velum maturation is characterized by coordinated repression of translational machinery and activation of metabolic and stress-adaptive pathways. These findings provide a systems-level framework for understanding the molecular mechanisms underlying flor yeast biofilm development during biological wine aging.
...
[26] Pisithkul, T., Schroeder, J. W., Trujillo, E. A., Yeesin, P., Stevenson, D. M., Chaiamarit, T., Coon, J. J., Wang, J. D., & Amador-Noguez, D. (2019). Metabolic Remodeling during Biofilm Development of Bacillus subtilis. mBio, 10(3). https://doi.org/10.1128/mBio.00623-19

[27] Purevdorj-Gage, B., Orr, M. E., Stoodley, P., Sheehan, K. B., & Hyman, L. E. (2007). role of FLO11 in Saccharomyces cerevisiae biofilm development in a laboratory based flow-cell system. FEMS Yeast Research, 7(3), 372–379. https://doi.org/10.1111/j.1567-1364.2006.00189.x

[28] Ishigami, M., Nakagawa, Y., Hayakawa, M., & Iimura, Y. (2004). FLO11 is essential for flor formation caused by the C-terminal deletion of NRG1 in Saccharomyces cerevisiae. FEMS Microbiology Letters, 237(2), 425–430. https://doi.org/10.1111/j.1574-6968.2004.tb09726.x
